...
{
    public class RoboticsPLPiston : PartModule, IJointLockState
    {
        ...
        [UI_FloatRange(maxValue = 100f, minValue = 0f, stepIncrement = 0.1f, scene = UI_Scene.Flight)]
        [KSPField(isPersistant = true, guiActive = true, guiName = "Target Extension", guiActiveEditor = false)]
        ...
        public void Move()
        {
            ...
            isMoving = true;
            targetPosition = axis * targetExtension;
            isFinished = false;
        }
        ...
        private void Start()
        {
            ...
            movingMeshTransform.localPosition = axis * targetExtension;
            UI_FloatRange range = (UI_FloatRange) Fields["targetExtension"].uiControlFlight;
            range.minValue = 0f;
            range.maxValue = maxExtension;
        }

        private void FixedUpdate()
        {
            if (axis * targetExtension != movingMeshTransform.localPosition &&
                axis * targetExtension != targetPosition)
            {
                Move();
            }
            ...
            {
                if (movingMeshTransform.localPosition == targetPosition)
                    isMoving = false;
                else
                    movingMeshTransform.localPosition = Vector3.MoveTowards(movingMeshTransform.localPosition,
                        targetPosition, speed * TimeWarp.deltaTime);
            }
        }
