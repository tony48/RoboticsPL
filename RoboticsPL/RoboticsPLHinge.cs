...
{
    public class RoboticsPLHinge : PartModule, IJointLockState
    {
        ...
        [UI_FloatRange(minValue = -90f, maxValue = 90f, stepIncrement = 1f, scene = UI_Scene.Flight)]
        ...
        /*private void Update()
        {
            if (isMoving && Events["Move"].guiActive)
            {
                this.Events["Move"].guiActive = false;
            }
            else if (!isMoving && !Events["Move"].guiActive)
            {
